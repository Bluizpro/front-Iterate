<template>
  <div class="form-container">
    <h2>Formulário de Baixa - Encomenda Correio</h2>
    <q-form @submit.prevent="darBaixaCorreio" class="q-gutter-md">
      <q-input
        v-model="nomeRecebedorCorreio"
...
// Dados do formulário
const nomeRecebedorCorreio = ref('');
// Verifica se é um único ID ou múltiplos IDs
const encomendaIdCorreio = ref(
...
watch(
  () => route.params.id,
  (newId) => {
    encomendaIdCorreio.value = Array.isArray(newId) ? newId : [newId];
  }
);

// Função para dar baixa na encomenda interna
const darBaixaCorreio = async () => {
  try {
    // Validação para garantir que o nome do recebedor não seja vazio
    if (!nomeRecebedorCorreio.value.trim()) {
      $q.notify({
        type: 'negative',
  ...
  try {
    ...
    for (const id of encomendaIdCorreio.value) {
      ...
      if (resposta && resposta.id) {
        $q.notify({
          type: 'positive',
          message: `Baixa realizada com sucesso para ${nomeRecebedorCorreio.value}!`,
        });
      } else {
...
const resetForm = () => {
  nomeRecebedorCorreio.value = ''; // Limpa o campo do nome do recebedor
